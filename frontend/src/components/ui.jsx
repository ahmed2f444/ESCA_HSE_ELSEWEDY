...
/** Read a CSS variable as an rgb() color string for use in inline styles. */
...
export function BarRow({ label, value, display, color = null, note, width }) {
  const c = color || themeRgb('--c-info')
  return (
    <div className="mb-3.5 last:mb-0">
      <div className="flex justify-between text-sm mb-0.5">
        <span>{label}</span>
        <b className="font-mono num font-semibold" style={{ color: c }}>
          {display ?? `${value}%`}
        </b>
      </div>
      <div className="bar-track" style={width ? { width } : undefined}>
        <i className="bar-fill" style={{ width: `${Math.min(value, 100)}%`, background: c }} />
      </div>
...
export function MiniBar({ value, color = null, width = 78 }) {
  const c = color || themeRgb('--c-info')
  return (
...
export function TimelineItem({ time, color = null, children }) {
  const c = color || themeRgb('--c-info')
  return (
...
export function Step({ n, title, tone, children }) {
  const bg = tone === 'ok' ? themeRgb('--c-safe') : tone === 'wn' ? themeRgb('--c-warn') : tone === 'nu' ? themeRgb('--c-steel3') : themeRgb('--c-hi')
  return (
    <div className="flex gap-3 py-3 border-b last:border-b-0" style={{ borderColor: `rgb(${getComputedStyle(document.documentElement).getPropertyValue('--c-line').trim().replace(/ /g,',')} / 0.5)` }}>
      <div
...
export function ErrorNote({ error, onRetry }) {
  return (
    <div className="p-4">
      <div className="border border-crit/40 bg-crit/10 rounded p-3.5 text-xs leading-7">
...
export function Donut({ value, pct, color = null }) {
  const c = color || themeRgb('--c-safe')
  return (
    <div className="flex items-center justify-center">
      <div
        className="w-24 h-24 rounded-full flex items-center justify-center relative print:border print:border-slate-300"
        style={{ background: `conic-gradient(${c} 0 ${pct}%, rgb(var(--c-steel3)) ${pct}% 100%)` }}
      >
...
export function LiveDot({ label = 'LIVE', tone = 'crit' }) {
  const c = tone === 'crit' ? themeRgb('--c-crit') : themeRgb('--c-safe')
  return (
