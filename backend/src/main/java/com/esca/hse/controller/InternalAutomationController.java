...
public class InternalAutomationController {
    ...
    public ResponseEntity<Map<String, Object>> action(
        ...
        jdbc.update("INSERT INTO automation_actions (action_record_id, event_id, idempotency_key, rule_id, entity_type, entity_id, action, alert_code, status, payload_json, processed_at_utc, audit_id) "
                ...
                new MapSqlParameterSource()
                        .addValue("action_id", actionId).addValue("event_id", eventId).addValue("key", key)
                        .addValue("rule_id", ruleId).addValue("entity_type", text(event, "entity_type"))
                        .addValue("entity_id", entityId).addValue("action", text(event, "action"))
                        .addValue("alert_code", text(event, "alert_code")).addValue("payload", String.valueOf(event.get("payload")))
                        .addValue("processed", java.sql.Timestamp.from(processed)).addValue("audit_id", auditId));
        Map<String, Object> response = new LinkedHashMap<>();
        response.put("status", "APPLIED"); response.put("event_id", eventId); response.put("action_record_id", actionId);
        response.put("audit_id", auditId); response.put("processed_at_utc", processed.toString());
    ...
    private boolean stillApplicable(String ruleId, Map<String, Object> entity, Map<String, Object> event) {
        ...
        return switch (ruleId) {
            case "AUT-001" -> Set.of("ACTIVE", "APPROVED").contains(status);
            case "AUT-002" -> !Set.of("REVOKED", "CANCELLED").contains(status) && date(entity.get("expiry_date")).isBefore(businessDate.plusDays(31));
            case "AUT-003" -> Set.of("OPEN", "IN_PROGRESS").contains(status) && date(entity.get("due_date")).isBefore(businessDate);
            case "AUT-004" -> Set.of("OPEN", "ACTIVE").contains(status) && number(entity.get("inherent_score")) >= 15;
            default -> false;
        };
    }
    ...
    private Map<String, Object> result(String status, Map<String, Object> row) {
        ...
        String timestamp = processed instanceof java.sql.Timestamp value ? value.toInstant().toString() : String.valueOf(processed);
        return Map.of("status", status, "event_id", row.get("event_id"), "action_record_id", row.get("action_record_id"),
